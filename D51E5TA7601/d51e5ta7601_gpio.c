...
static int dm_gpio_set_value(int *pin, int val)
{
    asm volatile("nop \n nop \n nop");
    gpio_put(*pin, val);
    asm volatile("nop \n nop \n nop");
...
static u16 read_id(struct d51e5ta7601_priv *priv)
{
    u16 id = 0;

    for (int i = 0; i < 16; i++) {
        gpio_put(priv->gpio.db[i], 1);
    }

    gpio_put(priv->gpio.rs, 0);
    asm volatile("nop \n");
    gpio_put(priv->gpio.cs, 0);

    gpio_put(priv->gpio.wr, 1);
    gpio_put(priv->gpio.rd, 0);
    sleep_us(10);

    for (int i = 0; i < 16; i++) {
        id <<= 1;
        id |= gpio_get(priv->gpio.db[i]);
    }
    gpio_put(priv->gpio.rd, 1);
    sleep_us(10);
...
static int d51e5ta7601_video_sync(struct d51e5ta7601_priv *priv)
{

    d51e5ta7601_set_addr_win(priv, 0, 0,
                         priv->display->xres - 1,
                         priv->display->yres - 1);
