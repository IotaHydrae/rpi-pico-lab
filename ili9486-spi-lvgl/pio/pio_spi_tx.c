...
#define PIO_CLK_DIV 6.f

static PIO g_pio = pio0;
static uint g_sm = 0;

void pio_spi_tx_set_rs(bool rs)
{
    gpio_put_masked((1u << PIN_RS), !!rs << PIN_RS);
}

void pio_spi_tx_set_cs(bool cs)
{
    gpio_put_masked((1u << PIN_CS), !!cs << PIN_CS);
}

void pio_spi_tx_set_rs_cs(bool rs, bool cs) {
    pio_spi_tx_set_rs(rs);
    pio_spi_tx_set_cs(cs);
}
...
static inline int __time_critical_func(pio_spi_tx_write8)(PIO pio, uint sm, uint8_t *buf, size_t len)
{
...
int pio_spi_write_buf_rs(uint8_t *buf, size_t len, int rs)
{
